...
ps=photos;
        ...
        Y.Array.each(ps,function(p){
        photoarray.push('http://farm'+p.farm+".static.flickr.com/"+p.server+"/"+p.id+"_"+p.secret+"_z.jpg?zz=1");
        },this);
        
        var ar=photoarray.join(',');
        if(typeof(Storage)!=="undefined" && !localStorage.urs)
        {
        
        localStorage.urs=ar;
        console.log(localStorage.urs)
        }
